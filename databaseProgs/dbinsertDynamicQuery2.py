...
try:
    with sqlite3.connect('Employee') as db: #using context manager
        ...
        # Inserting data with a special string formatter - ? for mapping data with column name in the table.
        # cursor.execute('''insert into emp values (?,?,?,?,?)''',(id, name, salary, age, dept)) #step4 ddl query
        # inserting data with dictionary - key value pair
        cursor.execute('''insert into emp values (:id,:name,:salary,:age,:dept)''',
